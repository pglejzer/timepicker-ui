...
function analyzeBundle(dir, label, friendlyNames = {}) {
  ...
  files.forEach((file) => {
    ...
    } else if (file.gzip > groupedFiles[key].gzip) {
      groupedFiles[key] = file;
    }
  });

  const deduplicatedFiles = Object.values(groupedFiles);

  const total = deduplicatedFiles.reduce(
    (acc, f) => ({
      raw: acc.raw + f.raw,
  ...
  return { label, files: deduplicatedFiles, total };
}
